...
public class ShapelessTableBuilder extends AbstractBuilder
{
    ...
    public ShapelessTableBuilder(IRecipeSerializer<?> type, String itemName, String textureName, int tier, int texX, int texY, int texWidth, int texHeight)
    {
    ...
    public void drawBackground(Screen screen, MatrixStack mstack, int builderX, int builderY)
    {
        if (tier == 4)
        {
            ...
            AbstractGui.blit(mstack, builderX, builderY, screen.getBlitOffset(), getTexX(), getTexY(), getTexWidth(), getTexHeight(), 512, 512);
        }
        else
        {
            super.drawBackground(screen, mstack, builderX, builderY);
        }
    }
    ...
    private class ShapelessTableResult implements IFinishedRecipe
    {
        ...
        public IRecipeSerializer<?> getType() { return ShapelessTableBuilder.this.getType(); }
